...
const ModalContent = ({ t, resolve, message }: { t: any; resolve: (val: boolean) => void; message: string }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4 z-[9999] pointer-events-none">
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bg-black/80 backdrop-blur-md transition-all duration-500 pointer-events-auto z-0 ${t.visible ? 'opacity-100' : 'opacity-0'
          }`}
        onClick={() => {
          toast.dismiss(t.id);
          resolve(false);
      ...
      <div
        ...
        className={`${t.visible ? 'animate-in fade-in zoom-in duration-300 scale-100' : 'animate-out fade-out zoom-out duration-200 scale-95'
      ...
      >
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 h-1 bg-[#36A886] rounded-b-full opacity-80" />

        <div className="p-8 pb-6">
          <div className="flex flex-col items-center text-center space-y-5">
            <div className="w-16 h-16 rounded-2xl bg-[#36A886]/10 flex items-center justify-center border border-[#36A886]/20 rotate-3 shadow-[0_0_20px_rgba(49,94,251,0.15)]">
              <svg className="h-8 w-8 text-[#36A886] -rotate-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="p-6 pt-2 flex gap-3">
          ...
          <button
            ...
            className="flex-1 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-white bg-[#36A886] hover:bg-[#2548D0] transition-all cursor-pointer shadow-[0_10px_20px_rgba(49,94,251,0.2)]"
          >
            Confirm
          </button>
        </div>
