...
const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar }) => {
  ...
  return (
    <nav className="fixed top-0 z-50 flex h-[5rem] w-full items-center justify-between bg-sidebar px-3 md:px-6 backdrop-blur-md">
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <Link href="/dashboard" className="flex h-10 w-20 md:w-32 items-center justify-start rounded cursor-pointer hover:opacity-90 transition-opacity">
          <Image src={logo} alt="Trustive Logo" className="invert-0 object-contain w-36" priority />
        </Link>
      </div>

      <div className="flex items-center gap-2 md:gap-4">
        <button className="hidden sm:flex items-center gap-2 rounded-xl bg-[#FAF7F2] px-3 md:px-5 py-2.5 md:py-3 text-[0.875rem] md:text-[1rem] font-medium text-[#0A0908] transition-colors hover:bg-zinc-200 cursor-pointer">
            ...
            return (
              <div className="flex items-center gap-4" {...(!ready && { 'aria-hidden': true, 'style': { opacity: 0, pointerEvents: 'none', userSelect: 'none' } })}>
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  {isProfileOpen && (
                    <div className="absolute right-0 mt-3 w-64 rounded-2xl bg-[#0F0E0D] border border-white/5 py-4 shadow-2xl backdrop-blur-xl">
                      {/* Mobile wallet section */}
                      <div className="lg:hidden px-4 py-3 border-b border-white/5 mb-3">
                        <div className="text-[0.75rem] text-zinc-500 uppercase font-bold tracking-wider mb-2 px-1">
                          Connect
                        </div>
                        {!connected ? (
                          <button onClick={openConnectModal} className="w-full flex items-center gap-2 rounded-xl bg-accent/10 px-4 py-2 text-[0.875rem] font-medium text-accent border border-accent/20 hover:bg-accent/20 transition-colors cursor-pointer">
                            <LuWallet className="h-4 w-4" />
                            Connect Wallet
                          </button>
                        ) : chain.unsupported ? (
                          <button onClick={openChainModal} className="w-full flex items-center gap-2 rounded-xl bg-red-500/10 px-4 py-2 text-[0.875rem] font-medium text-red-400 border border-red-500/20 hover:bg-red-500/20 transition-colors cursor-pointer">
                            <LuTriangleAlert className="h-4 w-4" />
                            Wrong Network
                          </button>
                        ) : (
                          <button onClick={openAccountModal} className="w-full flex items-center gap-2 rounded-xl bg-accent/10 px-4 py-2 text-[0.875rem] font-medium text-accent border border-accent/20 hover:bg-accent/20 transition-colors cursor-pointer">
                            <LuWallet className="h-4 w-4" />
                            {rbAccount.displayName}
                          </button>
                        )}
                      ...
                      <div className="px-2 space-y-1">
                        <Link
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[1rem] text-[#FAF7F2] hover:bg-white/5 transition-colors text-left group cursor-pointer"
                        >
                          <LuUser className="h-5 w-5 text-zinc-500 group-hover:text-accent transition-colors" />
                          Profile
                        </Link>
                        ...
                        {connected && (
                          <button
                            ...
                            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[1rem] text-[#FF4B4B] hover:bg-[#FF4B4B1A] transition-colors text-left group cursor-pointer"
                          >
                            <LuLogOut className="h-5 w-5 text-[#FF4B4B]" />
                            Disconnect Wallet
                          </button>
                        )}
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-[1rem] text-zinc-400 hover:bg-white/5 hover:text-white transition-colors text-left group cursor-pointer"
                        >
                          <LuLogOut className="h-5 w-5 text-zinc-500 group-hover:text-red-400" />
                          Logout Account
                        </button>
                      </div>
                    </div>
