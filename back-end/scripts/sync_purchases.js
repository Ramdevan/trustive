...
async function syncPurchases() {
    try {
        const conn = await mysql.createConnection({
            ...
            database: process.env.DB_NAME || 'ppm_crypto'
        });
        const provider = new ethers.JsonRpcProvider('https://ethereum-sepolia-rpc.publicnode.com');
        const icoAbi = JSON.parse(fs.readFileSync(path.join(__dirname, "../abi's/ico.json"), 'utf8'));
        const icoAddress = process.env.ICO_CONTRACT_ADDRESS || '0x8Ab0caB366B23Dcb88ceA447312CCb103B138cFa';
        ...
        for (const log of logs) {
            ...
            console.log(`Synced purchase: ${recipient} -> ${ppmTokens} Trustive (${paymentType})`);
        }
