        ...
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                <div className="flex flex-wrap items-center gap-4">
                    <button
                        ...
                        className="bg-[#E5B258] hover:bg-[#d6a345] text-black px-8 py-3.5 rounded-xl flex items-center gap-3 font-black text-xs uppercase tracking-widest transition-all shadow-lg hover:scale-105 active:scale-95 cursor-pointer"
                    >
                    ...
                    <button
                        ...
                        className="bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] text-white px-5 py-3.5 rounded-xl flex items-center gap-2 font-black text-xs uppercase tracking-widest transition-all cursor-pointer"
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-accent text-black uppercase font-black text-xs tracking-widest">
                                <th className="px-12 py-8 rounded-tl-[32px]">ID</th>
                                <th className="px-10 py-8">BEFORE</th>
                                <th className="px-10 py-8">AFTER</th>
                                <th className="px-10 py-8">TIMESTAMP</th>
                                <th className="px-12 py-8 rounded-tr-[32px]">Transaction Hash</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/[0.03]">
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={5} className="px-10 py-24 text-center text-zinc-600 font-medium italic">No ledger updates found.</td>
                                </tr>
                            ) : (
                                        ...
                                        <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                                            ...
                                            <td className="px-10 py-8">
                                                <div className="flex items-baseline gap-2">
                                                    <span className="text-zinc-400 font-bold text-lg font-mono">{formatUSD(rawOld)}</span>
                                                    <span className="text-xs text-zinc-600">/ Trustive</span>
                                                </div>
                                            </td>
                                            <td className="px-10 py-8">
                                                <div className="flex items-baseline gap-2">
                                                    <span className="text-[#E5B258] font-bold text-xl font-mono">{formatUSD(rawNew)}</span>
                                                    <span className="text-xs text-[#E5B258]/60">/ Trustive</span>
                                                </div>
                                            </td>
                                            <td className="px-10 py-8">
            ...
            {showModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl animate-in fade-in duration-300">
                    <div className="bg-[#0A0908] w-full max-w-lg rounded-[40px] border border-white/10 overflow-hidden shadow-2xl relative">
                        <div className="p-8 sm:p-10 space-y-6">

                            <div className="flex items-center gap-4">
                                <div className="w-14 h-14 bg-[#E5B258]/10 text-[#E5B258] rounded-[16px] flex items-center justify-center shrink-0 border border-[#E5B258]/20">
                                    <DollarSign className="w-6 h-6" />
                                </div>
                                <div className="flex flex-col min-w-0">
                                    <h2 className="text-2xl font-bold text-white tracking-tight truncate">Modify Token Valuation</h2>
                                    <p className="text-[9px] text-zinc-500 uppercase tracking-widest font-black mt-1">Update on-chain ICO pricing engine</p>
                                </div>
                            </div>

                            {/* Current Token Price Box */}
                            <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 space-y-2 overflow-hidden">
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest flex items-center gap-1.5">
                                        <Activity className="w-3.5 h-3.5 text-[#E5B258]" /> Current Token Price
                                    </span>
                                    <span className="text-[9px] px-2 py-0.5 rounded-md bg-[#E5B258]/10 text-[#E5B258] font-black uppercase tracking-widest border border-[#E5B258]/20">
                                        Active
                                    </span>
                                </div>
                                <div className="flex items-baseline justify-between pt-1 gap-4">
                                    <div className="min-w-0">
                                        <p className="text-3xl font-extrabold text-[#E5B258] font-mono tracking-tight truncate">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-500 text-lg font-bold">
                                        $
                                    </span>
                                    <input
                                        type="number"
                                        ...
                                        className="w-full bg-white/[0.02] border border-white/10 rounded-2xl pl-12 pr-24 py-5 text-white font-mono text-lg focus:outline-none focus:border-[#E5B258]/50 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none placeholder:text-zinc-700"
                                    />
                                    <span className="absolute right-6 top-1/2 -translate-y-1/2 text-zinc-500 text-xs font-bold uppercase tracking-wider">
                                        USD / Trustive
                                    </span>
                                </div>
                                ...
                                {previewTokensPerUSD !== null && (
                                    <div className="bg-[#E5B258]/5 border border-[#E5B258]/20 rounded-xl p-3.5 flex items-center justify-between text-xs animate-in fade-in duration-200">
                                        <span className="text-zinc-400 font-medium">Resulting Conversion:</span>
                                        <span className="font-mono font-bold text-[#E5B258]">
                        ...
                        <div className="p-8 bg-black flex gap-4 border-t border-white/5">
                            <button
                                onClick={() => setShowModal(false)}
                                disabled={isWriting}
                                className="flex-1 px-6 py-4 bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white rounded-2xl font-bold text-xs uppercase tracking-widest transition-colors cursor-pointer"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="flex-[2] px-6 py-4 bg-[#E5B258] hover:bg-[#d6a345] text-black rounded-2xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:scale-[1.02] active:scale-[0.98]"
                            >
                                {isWriting ? (
                                    <>
