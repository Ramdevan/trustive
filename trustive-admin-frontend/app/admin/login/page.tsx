...
export default function AdminLogin() {
    ...
    const handleWalletLogin = async () => {
        ...
        if (!isAdminAddress(address) && !isOwnerAddress(address)) {
            setError("The connected wallet is not an authorized Admin or Owner on the ICO contract.");
            return;
        }
        ...
        try {
            ...
            if (data.status) {
                ...
                if (token) {
                    ...
                    setSuccessMsg("Authenticated as CONTRACT ADMIN! Redirecting...");
                    setTimeout(() => {
    ...
    const isConnectedAdmin = isAdminAddress(address);
    ...
    return (
        <div className="min-h-screen bg-[#F4F4F6] flex flex-col items-center justify-center p-4 relative overflow-hidden">
            {/* Background Glow */}
            <div className="absolute -bottom-32 -right-32 w-80 h-80 bg-blue-200/40 rounded-full blur-3xl pointer-events-none" />

            <div className="w-full max-w-[460px] relative z-10 space-y-6">
                {/* Header */}
                <div className="text-center space-y-4">
                    <div className="relative inline-flex items-center justify-center w-16 h-16 bg-[#36A886]/10 border-2 border-[#36A886]/30 rounded-2xl rotate-6 shadow-xl shadow-blue-900/15">
                        <ShieldCheck className="w-8 h-8 text-[#36A886] -rotate-6" />
                    </div>
                    <div className="space-y-1">
                        <h1 className="text-3xl font-manrope font-extrabold text-[#001060] tracking-tight uppercase">
                            TRUSTIVE
                        </h1>
                        <p className="text-[#36A886] font-bold text-[11px] uppercase tracking-[4px] flex items-center justify-center gap-1.5">
                ...
                <div className="bg-white rounded-[32px] border-2 border-blue-200/80 p-8 shadow-[0_8px_30px_rgba(49,94,251,0.08)] space-y-6">
                    ...
                    {/* Section 1: Web3 Admin Wallet Quick-Login */}
                    <div className="p-4 bg-blue-50/50 border border-blue-200/70 rounded-2xl space-y-3">
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] font-bold text-[#36A886] uppercase tracking-wider flex items-center gap-1.5">
                                <Wallet className="w-3.5 h-3.5 text-[#36A886]" />
                                Admin Web3 Instant Login
                            </span>
                            {isConnected && (
                                <span
                                    className={cn(
                                ...
                                >
                                    {isConnectedAdmin ? "ADMIN WALLET" : isConnectedOwner ? "OWNER WALLET" : "UNAUTHORIZED"}
                                </span>
                            )}
                        </div>

                        {isConnected && address ? (
                            <div className="flex items-center justify-between text-xs font-mono bg-white px-3 py-2 rounded-xl border border-blue-200">
                            ...
                            <p className="text-xs text-zinc-600 leading-snug">
                                Connect any of the contract&apos;s 5 authorized Admin wallets for 1-click on-chain verification.
                            </p>
                        )}
                        ...
                        <button
                            ...
                            className="w-full py-3.5 rounded-xl font-bold uppercase tracking-wider text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md bg-[#36A886] hover:bg-[#2548D0] text-white shadow-[#36A886]/20 active:scale-95 disabled:opacity-50"
                        >
                            {walletLoggingIn ? (
                                <>
                                    <Loader2 className="w-4 h-4 animate-spin" />
                                    Verifying Admin Signature...
                                </>
                            ) : isConnected ? (
                                <>
                                    <ShieldCheck className="w-4 h-4" />
                                    Sign in as Contract Admin
                                </>
                            ) : (
                                <>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        {!require2FA ? (
                            <>
                                <div className="space-y-1.5">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:border-[#36A886] focus:ring-1 focus:ring-[#36A886] outline-none transition-all text-zinc-900 text-sm placeholder:text-zinc-400 font-medium"
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full px-4 py-3.5 pr-12 bg-zinc-50 border border-zinc-200 rounded-xl focus:border-[#36A886] focus:ring-1 focus:ring-[#36A886] outline-none transition-all text-zinc-900 text-sm placeholder:text-zinc-400 font-medium"
                                            placeholder="••••••••••••"
                                        />
                                        <button
                                            type="button"
                            ...
                            <div className="space-y-2 py-2 animate-in fade-in duration-300">
                                <label className="text-[10px] font-bold text-[#36A886] uppercase tracking-[4px] px-1 block text-center mb-3">
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-4 bg-zinc-50 border-2 border-[#36A886]/40 rounded-2xl focus:border-[#36A886] outline-none transition-all text-zinc-900 text-2xl text-center tracking-[0.8rem] font-bold placeholder:text-zinc-300"
                                    placeholder="000000"
                                />
                    ...
                    {/* Section 3: 5 Contract Admins List */}
                    <div className="pt-2 border-t border-zinc-100">
                        <button
                            type="button"
                            onClick={() => setShowAddresses(!showAddresses)}
                            className="w-full flex items-center justify-between text-[11px] font-bold text-[#36A886] hover:text-blue-700 transition-colors py-1 cursor-pointer uppercase tracking-wider"
                        >
                            <span>🛡️ 5 Authorized Admins (3/5 Quorum)</span>
                        ...
                        {showAddresses && (
                            <div className="mt-3 bg-zinc-50 rounded-2xl p-4 border border-zinc-200 text-xs space-y-2 animate-in fade-in duration-150">
                                <div className="flex items-center justify-between text-[10px] text-zinc-400 font-semibold mb-1">
                                    ...
                                    <span className="text-[#36A886]">BSC Testnet</span>
                                </div>
                                {CONTRACT_ADMINS.map((addr, idx) => (
                                    <div
                                        key={addr}
                                    ...
                                    >
                                        <div className="truncate flex-1 pr-2 text-zinc-700">
                                            <span className="font-bold text-[#36A886] font-sans mr-2">#{idx + 1}</span>
                                            {addr}
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => copyAddress(addr)}
                                            className="p-1 text-zinc-400 hover:text-[#36A886] transition-colors shrink-0"
                                            title="Copy address"
                                        >
                                            {copiedAddr === addr ? (
                ...
                <div className="text-center pt-2">
                    <Link
                        href="/owner/login"
                        className="inline-flex items-center gap-2 text-xs font-bold text-amber-700 hover:text-amber-800 uppercase tracking-wider transition-colors"
                    >
                        <Crown className="w-4 h-4 text-amber-500" />
                        Switch to Owner Governance Portal (/owner)
